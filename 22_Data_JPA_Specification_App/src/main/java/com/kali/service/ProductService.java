...
import com.kali.spec.ProductSepcifications;
...
public class ProductService {
	...
	public void getProducts() {
		...
		Example<Product> of = Example.of(p); //to filter the data
		
		List<Product> findAll = productRepo.findAll(of);
	...
	public void getProductsSpec(String name, Double minPrice, Double maxPrice) {
		...
		if(name!=null) {
			spec=spec.and(ProductSepcifications.nameLike(name));
		}
		
		if(minPrice!=null) {
			spec = spec.and(ProductSepcifications.priceGreaterThan(minPrice));
		}
		
		if(minPrice!=null) {
			spec = spec.and(ProductSepcifications.priceLessThan(maxPrice));
		}
